...
class ProductProduct(models.Model):
    ...
    @api.multi
    def _set_standard_cost_price(self, value):
    ...
    def create(self, values):
        ...
        product._set_standard_cost_price(values.get('standard_price', 0.0))
        return product

    @api.multi
    def write(self, values):
        if self.env.context.get('bypass_down_write', False):
            return super(ProductProduct, self).write(values)
        if 'standard_price' in values:
            for product in self:
                product._set_standard_cost_price(values['standard_price'])
